...
public final class DateUtil {
    ...
    private static final String DATE_TIME_PATTERN_T_Z = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'";

    public static SimpleDateFormat getDTPatternTZ() {
        return new SimpleDateFormat(DATE_TIME_PATTERN_T_Z);
    }

    public static String getDateTimePatternTZ(Date date) {
        if (date == null) {
            return "";
        }
        return getDTPatternTZ().format(date);
    }

    public static Date strToDateTimeTZ(String strDate) {
        try {
            return getDTPatternTZ().parse(strDate);
        } catch (Exception e) {
            return null;
        }
    }

    private static final String DATE_TIME_PATTERN_YMDHM = "yyyy-MM-dd HH:mm";

    public static SimpleDateFormat getDFPatternYMDHM() {
        return new SimpleDateFormat(DATE_TIME_PATTERN_YMDHM);
    }
    ...
    public static String getDateYMDHM(Date date) {
        ...
        return getDFPatternYMDHM().format(date);
    }

    public static Date strToDateYMDHM(String strDate) {
        try {
            return getDFPatternYMDHM().parse(strDate);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    private static final String DATE_PATTERN_POINT_YM = "yyyy.MM";

    public static SimpleDateFormat getDFPatternPointYM() {
        return new SimpleDateFormat(DATE_PATTERN_POINT_YM);
    }

    public static String getDatePointYM(Date date) {
        if (date == null) {
            return "";
        }
        return getDFPatternPointYM().format(date);
    }
    ...
    // 获取某个时刻的00分
    public static Date getDateFirstTimeSend(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        calendar.set(Calendar.SECOND, 0);
        calendar.set(Calendar.MILLISECOND, 0);
    ...
    public static int daysBetweenAbove(Date d1, Date d2) {
        BigDecimal result = BigDecimal.valueOf(Math.abs(d2.getTime() - d1.getTime())).divide(BigDecimal.valueOf(1000 * 86400), 4, BigDecimal.ROUND_FLOOR);
        double ceil = Math.ceil(result.doubleValue());
